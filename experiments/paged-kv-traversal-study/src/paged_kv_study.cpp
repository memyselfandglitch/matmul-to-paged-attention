...
enum class Stage { Fixed, Full };
enum class BlockOrder { Sequential, Shuffled };
...
void print_help(const char* program) {
  std::cout
      ...
      << "  --stage fixed|full   fixed: default BNHD/NHD memory;\n"
      << "                       full: all 3 memory x 3 traversal combinations\n"
      << "  --block-order sequential|shuffled (default sequential)\n"
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (option == "--stage") {
      if (value == "fixed") {
        ...
        throw std::invalid_argument("--stage must be fixed or full");
      }
      ...
      if (value == "sequential") {
        ...
        throw std::invalid_argument(
            "--block-order must be sequential or shuffled");
      }
    } else if (option == "--csv") {
...
int main(int argc, char** argv) {
  try {
    ...
    std::cout << "Paged KV traversal study (B means physical block, not batch)\n"
              ...
              << "Block table: "
              << (config.block_order == BlockOrder::Sequential ? "sequential"
                                                               : "shuffled")
              << "\n"
              << "Stage: "
              << (config.stage == Stage::Fixed ? "fixed BNHD/NHD memory"
                                               : "full 3x3")
              << "\n\n";
